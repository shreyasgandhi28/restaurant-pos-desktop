...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600">Table <?php echo e($order->restaurantTable->table_number); ?> • Server: <?php echo e($order->user->name); ?></p>
            </div>
            <div class="flex space-x-2">
                ...
                <a href="<?php echo e(route('pos.index')); ?>" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2 px-4 rounded text-sm transition">
                    Back to POS
                </a>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow mb-6 p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-6">
            <div class="text-center">
                ...
                <p class="font-bold text-lg text-indigo-600"><?php echo e($order->order_number); ?></p>
            </div>
            <div class="text-center">
                <p class="text-gray-500 mb-1">Table</p>
                <p class="font-bold"><?php echo e($order->restaurantTable->table_number); ?></p>
            </div>
            <div class="text-center">
                <p class="text-gray-500 mb-1">Server</p>
                <p class="font-bold"><?php echo e($order->user->name); ?></p>
            </div>
            <div class="text-center">
        ...
        <?php if($order->notes): ?>
            <div class="pt-4 border-t">
                ...
                <p class="text-gray-800 bg-gray-50 p-3 rounded-lg"><?php echo e($order->notes); ?></p>
            </div>
        <?php endif; ?>
    </div>
                    ...
                    <div class="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                        <div class="flex items-center space-x-4 flex-1">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600"><?php echo e($item->menuItem->category->name); ?></p>
                                <?php if($item->special_instructions): ?>
                                    <p class="text-sm text-indigo-600 mt-1">Note: <?php echo e($item->special_instructions); ?></p>
                                <?php endif; ?>
                                <div class="mt-2">
                                    <select onchange="updateItemStatus(<?php echo e($item->id); ?>, this.value)"
                                        ...
                                        <option value="preparing" <?php echo e($item->status == 'preparing' ? 'selected' : ''); ?>>Preparing</option>
                                        <option value="ready" <?php echo e($item->status == 'ready' ? 'selected' : ''); ?>>Ready</option>
                                        <option value="served" <?php echo e($item->status == 'served' ? 'selected' : ''); ?>>Served</option>
                                        <option value="cancelled" <?php echo e($item->status == 'cancelled' ? 'selected' : ''); ?>>Cancelled</option>
                                    </select>
                                </div>
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span class="font-semibold" data-order-total="subtotal">₹<?php echo e(number_format($order->subtotal, 2)); ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Tax (<?php echo e($order->bill ? $order->bill->tax_percentage : 10); ?>%):</span>
                    <span class="font-semibold" data-order-total="tax">₹<?php echo e(number_format($order->tax, 2)); ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Service Charge:</span>
                    <span class="font-semibold" data-order-total="service_charge">₹<?php echo e(number_format($order->service_charge, 2)); ?></span>
                </div>
                ...
                <?php if($order->bill && $order->bill->discount_amount > 0): ?>
                    <div class="flex justify-between py-2 border-b border-gray-100 text-green-600">
                        ...
                        <span class="font-semibold">-₹<?php echo e(number_format($order->bill->discount_amount, 2)); ?></span>
                    </div>
                <?php endif; ?>
                <div class="flex justify-between py-3 border-t-2 border-gray-300 text-lg font-bold">
                    <span>TOTAL:</span>
                    <span class="text-indigo-600" data-order-total="total">₹<?php echo e(number_format($order->total, 2)); ?></span>
                </div>
            </div>
            ...
            <?php if($order->bill): ?>
                <div class="mt-6 pt-4 border-t">
                    <div class="bg-gray-50 rounded-lg p-4 text-center">
                        ...
                        <p class="font-bold text-xl text-indigo-600"><?php echo e($order->bill->bill_number); ?></p>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium mt-2
                            <?php if($order->bill->status == 'pending'): ?> bg-yellow-100 text-yellow-800
                            <?php elseif($order->bill->status == 'paid'): ?> bg-green-100 text-green-800
    ...
    <?php if($order->kitchenOrderTickets && $order->kitchenOrderTickets->count() > 0): ?>
        <div class="mt-6 bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Kitchen Order Tickets (KOTs)</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <?php $__currentLoopData = $order->kitchenOrderTickets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="border-2 <?php echo e($kot->printed_at ? 'border-gray-300' : 'border-indigo-300'); ?> rounded-lg p-4 <?php echo e($kot->printed_at ? 'bg-gray-50' : 'bg-indigo-50'); ?>">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                                <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                </svg>
                                <span class="font-bold text-gray-900"><?php echo e($kot->kot_number); ?></span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-sm px-2 py-1 rounded-full font-semibold <?php echo e($kot->printed_at ? 'bg-gray-100 text-gray-800' : 'bg-indigo-100 text-indigo-800'); ?>"><?php echo e($kot->status); ?></span>
                                <?php if(!$kot->printed_at): ?>
                                    <button onclick="printKOT(<?php echo e($kot->id); ?>)" class="text-sm bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded font-medium transition">
                                        Print
                                    </button>
                                <?php endif; ?>
                            </div>
                        ...
                        <ul class="text-sm space-y-1">
                            <?php $__currentLoopData = $kot->orderItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex justify-between text-gray-700">
                                    <span><?php echo e($item->quantity); ?>x <?php echo e($item->menuItem->name); ?></span>
                                    <span class="text-gray-600">₹<?php echo e(number_format($item->total_price, 2)); ?></span>
                                </li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ul>
                        <?php if($kot->printed_at): ?>
                            <p class="text-xs text-gray-500 mt-2">Printed: <?php echo e($kot->printed_at->format('M d, Y H:i')); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
...
<script>
let currentOrder = <?php echo json_encode($order, 15, 512) ?>;
let authToken = '<?php echo e(auth()->user()->createToken("pos-token")->plainTextToken); ?>';
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\restaurant-pos-desktop\backend\resources\views\orders\pos.blade.php ENDPATH**/ ?>