...
<div class="flex justify-end space-x-1">
    <?php if(!$order->bill && in_array($order->status, ['pending', 'preparing', 'ready', 'served'])): ?>
        <form id="generateBillForm-<?php echo e($order->id); ?>" action="<?php echo e(route('bills.store')); ?>" method="POST" class="inline">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="order_id" value="<?php echo e($order->id); ?>">
            <input type="hidden" name="discount_percentage" id="discount_percentage-<?php echo e($order->id); ?>" value="0">
            <input type="hidden" name="discount_reason" id="discount_reason-<?php echo e($order->id); ?>" value="">
            
            <button type="button" 
                    onclick="openBillDialog('<?php echo e($order->id); ?>')"
    ...
    <?php elseif($order->bill): ?>
        ...
        <a href="javascript:void(0)" 
           onclick="openBillPrintWindow('<?php echo e(URL::signedRoute('bills.print', $order->bill)); ?>')"
           title="Print"
        ...
        <a href="<?php echo e(route('bills.download', $order->bill)); ?>" 
           title="Download PDF"
           class="inline-flex items-center px-2 py-1 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-indigo-500">
            <svg class="h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<?php $__env->startPush('modals'); ?>
    <div id="billDialog-<?php echo e($order->id); ?>" class="fixed inset-0 z-50 hidden" aria-labelledby="billDialogTitle-<?php echo e($order->id); ?>" role="dialog" aria-modal="true">
        <div class="fixed inset-0 bg-gray-900 bg-opacity-50" onclick="closeBillDialog('<?php echo e($order->id); ?>')"></div>
        <div class="relative z-10 flex min-h-screen items-center justify-center px-4">
            <div class="w-full max-w-sm rounded-lg bg-white shadow-xl">
                <div class="flex items-center justify-between border-b px-4 py-3">
                    <div>
                        <h3 id="billDialogTitle-<?php echo e($order->id); ?>" class="text-base font-semibold text-gray-900">Generate Bill</h3>
                        <p class="text-xs text-gray-500">Add an optional discount before generating the bill.</p>
                    </div>
                    <button type="button" onclick="closeBillDialog('<?php echo e($order->id); ?>')" class="text-gray-400 hover:text-gray-600 focus:outline-none" aria-label="Close dialog">&times;</button>
                </div>
                <div class="space-y-3 px-4 py-4">
                    <div>
                        <label for="billDialogDiscount-<?php echo e($order->id); ?>" class="block text-xs font-medium text-gray-700">Discount Percentage</label>
                        <input type="number" id="billDialogDiscount-<?php echo e($order->id); ?>" min="0" max="100" step="0.01" value="0"
                               class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label for="billDialogReason-<?php echo e($order->id); ?>" class="block text-xs font-medium text-gray-700">Discount Reason <span class="text-[10px] text-gray-400">(required when discount &gt; 0)</span></label>
                        <textarea id="billDialogReason-<?php echo e($order->id); ?>" rows="2"
                                  class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                  placeholder="Enter reason"></textarea>
                    </div>
                    <p id="billDialogError-<?php echo e($order->id); ?>" class="hidden rounded bg-red-50 px-2 py-1 text-xs text-red-600"></p>
                </div>
                <div class="flex items-center justify-end gap-2 border-t bg-gray-50 px-4 py-3">
                    <button type="button" onclick="closeBillDialog('<?php echo e($order->id); ?>')" class="rounded border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100">Cancel</button>
                    <button type="button" onclick="submitBillDialog('<?php echo e($order->id); ?>')" class="rounded bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Generate</button>
                </div>
            </div>
        </div>
    </div>
...
async function openBillPrintWindow(url) {
    // Open the print window immediately (like KOT does)
    ...
    printWindow.document.write('<html><body style="font-family: sans-serif; text-align: center; padding: 50px 20px;"><p>Loading bill...</p></body></html>');
    ...
    try {
        ...
        printWindow.document.open();
        printWindow.document.write('<html><body style="font-family: sans-serif; text-align: center; padding: 50px 20px;"><p style="color: red;">Failed to load bill. Please try again.</p></body></html>');
        printWindow.document.close();
    }
}
</script>
<?php $__env->stopPush(); ?>
<?php /**PATH D:\restaurant-pos-desktop\backend\resources\views/components/bill/generate-button.blade.php ENDPATH**/ ?>