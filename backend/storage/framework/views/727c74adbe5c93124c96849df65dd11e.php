    ...
    <section class="flex-1 min-w-0">
      <div class="bg-white rounded-lg shadow p-6 mb-6">
                    ...
                    <?php $__currentLoopData = $menuItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="menu-item bg-white border border-gray-200 rounded-lg p-2 cursor-pointer hover:border-indigo-500 hover:shadow-sm transition flex flex-col" 
                            ...
                            <div class="mt-3 flex flex-col">
                                <h4 class="text-sm font-medium text-gray-900 line-clamp-2 leading-snug"><?php echo e($item->name ?? 'Unnamed Item'); ?></h4>
                                <p class="text-sm font-bold text-indigo-600 whitespace-nowrap mt-1">₹<?php echo e(number_format($item->price ?? 0, 0)); ?></p>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
        ...
        <aside class="w-[380px] min-w-[320px] bg-white rounded-lg shadow p-4 overflow-y-auto" style="max-height: calc(100vh - 3rem);">
            ...
            <aside id="ongoingOrders" class="w-[320px] min-w-[280px] hidden bg-white rounded-lg shadow p-4 overflow-y-auto" style="max-height: calc(100vh - 3rem);">
                <div class="flex items-center justify-between mb-3">
...
async function loadTableOrders(tableId) {
    try {
        ...
        if (table.orders && table.orders.length > 0) {
            ...
            adjustMenuGrid(); // Expand menu grid when no ongoing orders
        }
    } catch (error) {
...
function displayOngoingOrders(order) {
    ...
    // Adjust menu grid for better visibility with ongoing orders panel
    adjustMenuGrid();
}

// Adjust menu grid columns based on ongoing orders visibility and screen size
function adjustMenuGrid() {
    ...
    menuGrid.style.transition = 'all 0.3s ease-in-out';
    
    // Get the actual available width for the menu grid
    const menuContainer = menuGrid.parentElement;
    ...
    // Determine grid classes based on ongoing orders visibility and screen size
    let gridClasses = 'grid gap-4';
    
    if (ongoingOrders && !ongoingOrders.classList.contains('hidden')) {
        // Ongoing orders visible - use fewer columns, but respect screen size
        const cols = Math.min(maxPossibleColumns, 4); // Cap at 4 columns max
        ...
        if (cols <= 1) {
            ...
            gridClasses += ' grid-cols-2 sm:grid-cols-2 lg:grid-cols-3';
        } else {
            gridClasses += ' grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4';
        }
    } else {
        // No ongoing orders - use more columns, but still respect screen size
        const cols = Math.min(maxPossibleColumns, 6); // Cap at 6 columns max
    ...
    menuGrid.className = gridClasses;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('resize', function() {
        ...
        resizeTimeout = setTimeout(function() {
            adjustMenuGrid();
        }, 250); // Adjust after 250ms of no resize events
    });
});
