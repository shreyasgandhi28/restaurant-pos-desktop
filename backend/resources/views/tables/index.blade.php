...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="flex space-x-2">
            ...
            <a href="{{ route('tables.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
                Add New Table
            </a>
        </div>
        @endrole
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @foreach($tables as $table)
            <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition hover:scale-105">
                <div class="p-6 
                    @if($table->effective_status == 'available') bg-gradient-to-br from-green-400 to-green-600
                    @elseif($table->effective_status == 'occupied') bg-gradient-to-br from-red-400 to-red-600
                    @else bg-gradient-to-br from-yellow-400 to-yellow-600
                    @endif">
                    <div class="text-center text-white">
                        <div class="text-4xl font-bold mb-2">{{ $table->table_number }}</div>
                        <div class="text-sm opacity-90">Capacity: {{ $table->capacity }} persons</div>
                    </div>
                </div>
                ...
                <div class="p-4 bg-white">
                    <div class="mb-3">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                            @if($table->effective_status == 'available') bg-green-100 text-green-800
                            @elseif($table->effective_status == 'occupied') bg-red-100 text-red-800
                            @else bg-yellow-100 text-yellow-800
                            @endif">
                            {{ ucfirst($table->effective_status) }}
                        </span>
                    </div>

                    @if($table->effective_status == 'available')
                    ...
                    @elseif($table->effective_status == 'occupied')
                        @php
                        ...
                        @if($currentOrder)
                            <div class="space-y-2">
                                ...
                                <a href="{{ route('bills.create', ['order' => $currentOrder->id]) }}" 
                                   class="block w-full text-center bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded transition text-sm">
                                    Generate Bill
                                </a>
                            </div>
                        @endif
                    ...
                    <div class="mt-4 flex justify-between items-center text-sm text-gray-500">
                        ...
                        @role('admin|manager')
                        <div class="space-x-2">
                            <a href="{{ route('tables.edit', $table) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
    ...
    @if($tables->isEmpty())
        <div class="bg-white rounded-lg shadow p-12 text-center">
            ...
            @role('admin')
            <div class="mt-6">
                <a href="{{ route('tables.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                    Add New Table
                </a>
            </div>
            @endrole
...
async function fixTableStatuses() {
    ...
    try {
        const response = await fetch('/api/tables/recalculate-statuses', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${document.querySelector('meta[name="api-token"]')?.getAttribute('content') || '{{ auth()->user()->createToken("pos-token")->plainTextToken }}'}`,
