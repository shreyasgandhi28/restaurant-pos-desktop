...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Manage your restaurant staff members</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('staff-salary-advances.index') }}" class="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2 px-4 rounded transition text-sm flex items-center shadow-sm">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
                Back
            </a>
            <button onclick="document.getElementById('addStaffForm').classList.toggle('hidden')" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded transition">
                Add New Staff
            </button>
        </div>
    </div>
    ...
    <div id="addStaffForm" class="hidden mb-8 bg-white rounded-lg shadow overflow-hidden">
        <div class="p-6">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                ...
                                <form id="delete-form-{{ $member->id }}" action="{{ route('staff.destroy', $member) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('delete-form-{{ $member->id }}', 'Are you sure you want to delete this staff member?')"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            ...
                            <div class="mt-6">
                                <button onclick="document.getElementById('addStaffForm').classList.remove('hidden')" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                                    Add Staff
                                </button>
                            </div>
                        </td>
